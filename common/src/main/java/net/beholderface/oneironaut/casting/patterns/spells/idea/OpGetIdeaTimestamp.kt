    ...
    override fun execute(args: List<Iota>, env: CastingEnvironment): List<Iota> {
        ...
        if (rawKeyIota.type == EntityIota.TYPE){
            ...
            if (keyEntity.isPlayer || keyEntity.type.equals(EntityType.VILLAGER)){
                output = IdeaInscriptionManager.getIotaTimestamp(keyEntity.uuid, env.world)
            } else {
            ...
            output = IdeaInscriptionManager.getIotaTimestamp(keyPos, env.world)
        } else if (rawKeyIota.type == SoulprintIota.TYPE){
            val keySoulprint = args.getSoulprint(0, argc).toString() + "soul"
            output = IdeaInscriptionManager.getIotaTimestamp(keySoulprint, env.world)
        } else {
